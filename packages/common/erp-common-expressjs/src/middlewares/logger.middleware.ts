...
export const loggerMiddleware = async (request: Request, response: Response, next: NextFunction) => {
	...
	Winston.logger.info('logger', { requestId: Context.get('requestId'), auth: JSON.stringify(auth), action: originalUrl, method, payload: JSON.stringify({ body, params, query }) });
	next();
};