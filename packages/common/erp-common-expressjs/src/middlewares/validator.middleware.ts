...
export const validatorMiddleware = (schema: object) => (request: Request, response: Response, next: NextFunction) => {
	...
	if (!valid(data)) {
		...
		Winston.logger.error(TypeErrorMessage.VALIDATION, { requestId: Context.get('requestId'), auth: JSON.stringify(auth), action: originalUrl, method, payload: JSON.stringify({ body, params, query }), errors: JSON.stringify(errors) });
