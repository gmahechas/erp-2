...
export const createOneOffice = async (
  data: ICreateOffice,
): Promise<IOffice> => {
...
export const updateOneOffice = async (
  data: IUpdateOffice,
...
export const deleteOneOffice = async (
  data: IDeleteOffice,
): Promise<IOffice | null> => {
  const { companyId } = Context.get('auth');
  const { officeId } = data;
...
export const searchOneOffice = async (
  data: Partial<ISearchOffice>,
): Promise<IOffice | null> => {
  const { companyId } = Context.get('auth');
  const newData = Object.assign({}, data, { companyId });
...
export const searchManyOffice = async (
  data: Partial<ISearchOffice>[],
): Promise<IOffice[]> => {
  const { companyId } = Context.get('auth');
  const newData =
