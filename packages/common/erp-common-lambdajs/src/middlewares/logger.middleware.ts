...
export const loggerMiddleware = async (event: APIGatewayProxyEvent) => {
	...
	Winston.logger.info('logger', { requestId: Context.get('requestId'), auth: JSON.stringify(auth), action: path, method: httpMethod, payload: JSON.stringify({ body, params: pathParameters, query: queryStringParameters }) });
};