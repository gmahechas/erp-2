...
export const validatorMiddleware = async (schema: object) => (event: APIGatewayProxyEvent) => {
	...
	if (!valid(data)) {
		...
		Winston.logger.error(TypeErrorMessage.VALIDATION, { requestId: Context.get('requestId'), auth: JSON.stringify(auth), action: path, method: httpMethod, payload: JSON.stringify({ body, params: pathParameters, query: queryStringParameters }), errors: JSON.stringify(errors) });
