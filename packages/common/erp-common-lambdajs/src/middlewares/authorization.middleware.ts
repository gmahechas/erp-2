...
export const authorizationMiddleware = async (scopes: string[]) => (event: APIGatewayProxyEvent) => {
	...
	for (const capability of scopes) {
		...
		if (!scope[service]) {
			Winston.logger.error(TypeErrorMessage.AUTHORIZATION, { requestId: Context.get('requestId'), auth: JSON.stringify(auth), action: path, method: httpMethod, payload: JSON.stringify({ body, params: pathParameters, query: queryStringParameters }) });
