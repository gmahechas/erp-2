...
export const createOneGroupV1 = async (data: ICreateGroup): Promise<IGroup> => {
  ...
  const { data: response } = await axiosClient(env['ms-1']!.app!.endpoint!).post<{ data: IGroup }>(
    '/rest/v1/1/group/create/one',
    data,
    {
      headers: {
...
export const updateOneGroupV1 = async (data: IUpdateGroup): Promise<IGroup | null> => {
  const token = Context.get('token');
  const requestId = Context.get('requestId');
  const { data: response } = await axiosClient(env['ms-1']!.app!.endpoint!).put<{ data: IGroup }>(
    '/rest/v1/1/group/update/one',
    data,
    {
      headers: {
        'Content-Type': 'application/json',
...
export const deleteOneGroupV1 = async (data: IDeleteGroup): Promise<IGroup | null> => {
  const token = Context.get('token');
  const requestId = Context.get('requestId');
  const { data: response } = await axiosClient(env['ms-1']!.app!.endpoint!).delete<{ data: IGroup }>(
    '/rest/v1/1/group/delete/one',
    {
      data,
      headers: {
        'Content-Type': 'application/json',
