...
export const createOneCompany = async (data: ICreateCompany): Promise<ICompany> => {
	const { data: response } = await axiosClient(env.ms!.four!.url!).post<{ data: ICompany }>('/rest/v1/4/company/create/one', data);
	return response.data;
};

export const updateOneCompany = async (data: IUpdateCompany): Promise<ICompany | null> => {
	const { data: response } = await axiosClient(env.ms!.four!.url!).put<{ data: ICompany }>('/rest/v1/4/company/update/one', data);
	return response.data;
};

export const deleteOneCompany = async (data: IDeleteCompany): Promise<ICompany | null> => {
	const { data: response } = await axiosClient(env.ms!.four!.url!).delete<{ data: ICompany }>('/rest/v1/4/company/delete/one', { data });
	return response.data;
};

export const searchOneCompany = async (data: Partial<ISearchCompany>): Promise<ICompany | null> => {
	const { data: response } = await axiosClient(env.ms!.four!.url!).post<{ data: ICompany }>('/rest/v1/4/company/search/one', data);
	return response.data;
};

export const searchManyCompany = async (data: Partial<ISearchCompany>[]): Promise<ICompany[]> => {
	const { data: response } = await axiosClient(env.ms!.four!.url!).post<{ data: ICompany[] }>('/rest/v1/4/company/search/many', data);
	return response.data;
};