...
export const searchOneUser = async (
...
export const searchManyUser = async (
  data: Partial<ISearchUser>[],
): Promise<IUser[]> => {
  const { companyId, companyKey } = Context.get('auth');
  const newData =
    data.length === 0
      ...
      : data.map((item) => Object.assign({}, item, { companyId, companyKey }));
  const search = newData.length === 0 ? [{}] : newData;
  const result = await User.find({ $or: search });
  return result;
};

export const searchOneUserToSignin = async (
  data: Partial<ISearchUser> | { companyKey: string },
): Promise<IUser | null> => {
